...
import TimePopover from '@choerodon/master/lib/containers/components/c7n/components/time-popover';
...
const TemplateTable = observer((props) => {
  ...
  const handleCreateAppTemplate = useCallback((record?: Record) => {
    ...
    Modal.open({
      ...
      key: Modal.key(),
      children: <AddTemplate refresh={refresh} templateId={templateId} />,
      drawer: true,
  ...
  const handlePermission = useCallback(async ({ record }) => {
    ...
    try {
      if (await TemplateServices.addTemplatePermission(record.get('id'), organizationId)) {
        refresh();
        return true;
      }
      return false;
  ...
  const renderName = useCallback(({ value, record }) => {
    // type 为P：预定义；C: 自定义
    if (record.get('type') === 'P' || record.get('status') !== 'S') {
