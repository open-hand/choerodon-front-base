  ...
  const handleSubmit = () => {
    ...
    props.form.validateFields((err, values) => {
      if (!err) {
        ...
        UserInfoStore.updateUserInfo(newUser)
          .then((data) => {
            if (data.failed) {
 ...
 */
  ...
  const renderLanguageOptions = () => [
    <Option key="zh_CN" value="zh_CN">
      <FormattedMessage id={`${intlPrefix}.language.zhcn`} />
    </Option>,
  ...
  const renderTimeZoneOptions = () => {
    ...
    if (timeZone.length > 0) {
      return timeZone.map(({ code, description }) => (
    ...
    return [
      <Option key="CTT" value="CTT">
        <FormattedMessage id={`${intlPrefix}.timezone.ctt`} />
      </Option>,
  ...
  const checkEmailAddress = (rule, value, callback) => {
    if (value !== user.email && value !== "") {
      UserInfoStore.checkEmailAddress(value)
        .then(({ failed }) => {
          if (failed) {
            callback(
              intl.formatMessage({ id: `${intlPrefix}.email.used.msg` })
            );
          } else {
            callback();
          }
    ...
    if (value) {
      if (pattern.test(value)) {
        if (code === "86") {
          pattern = /^1[3-9]\d{9}$/;
          if (pattern.test(value)) {
            UserInfoStore.checkPhoneExist(value)
              .then((res) => {
                if (res) {
                  callback(
                    intl.formatMessage({ id: `${intlPrefix}.phone.used.msg` })
                  );
                } else {
                  callback();
                }
            ...
            callback(
              formatMessage({ id: `${intlPrefix}.phone.district.rule` })
            );
          }
        } else {
          callback();
            ...
            rules: [
              {
                ...
                message: intl.formatMessage({
                  id: `${intlPrefix}.name.require.msg`,
                }),
              },
            ],
            ...
            rules: [
              {
                ...
                // whitespace: true,
                message: intl.formatMessage({
                  id: `${intlPrefix}.email.require.msg`,
                }),
              },
              {
                type: "email",
                message: intl.formatMessage({
                  id: `${intlPrefix}.email.pattern.msg`,
                }),
              },
              {
