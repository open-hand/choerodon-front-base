...
export default ({ orgId, name }) => {
  ...
  return {
    ...
    fields: [
      ...
      {
        name: 'serverAddress', type: 'string', label: '主机名', required: true,
      }, // 必填
      {
        name: 'useSSL', type: 'boolean', label: '是否使用SSL', defaultValue: false,
      },
      {
        name: 'port', type: 'number', label: '端口号', required: true,
      }, // 必填
      { name: 'enabled', type: 'boolean' },
      {
        name: 'sagaBatchSize', type: 'number', label: '同步用户saga发送用户数量', min: 1, step: 1, defaultValue: 500, required: true,
      }, // 必填
      {
      ...
      {
        name: 'account', type: 'string', label: '管理员登录名', help: '用户登录到 LDAP。例如：user@domain.name 或 cn =用户, dc =域、dc =名称', required: true,
      }, // 必填
      {
        name: 'ldapPassword', type: 'string', label: '管理员密码', required: true,
      }, // 必填
      {
        name: 'objectClass', type: 'string', label: '用户对象类', help: '仅支持单个objectclass', required: true,
