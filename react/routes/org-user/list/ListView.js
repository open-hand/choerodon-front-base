...
const InviteModal = C7NTryImport(
  "@choerodon/base-business/lib/routes/invite-user-manage"
);
...
export default withRouter(
  observer((props) => {
    ...
    async function handleDisable(record) {
      try {
        await axios.put(
          `/iam/choerodon/v1/organizations/${organizationId}/users/${record.get(
            "id"
          )}/disable`
        );
    ...
    async function handleEnable(record) {
      try {
        ...
        if (HAS_BASE_PRO) {
          const res = await OrgUserServices.axiosGetCheckEnableUser(
            organizationId
          );
          // 可创建
          if (res.enableUser) {
    ...
    function handleCreate() {
      ...
      if (
        passwordPolicyDataSet.current &&
        passwordPolicyDataSet.current.get("enablePassword")
      ) {
    ...
    async function handleSyncSetting() {
      ...
      const syncSetting = Modal.open({
        ...
        children: (
          <LdapModal
            linkLDAP={(instance) => linkToLDAP(instance)}
            ldapId={res.id}
          />
        ),
        okText: "手动同步",
        ...
        footer: (okBtn, cancelBtn) => (
          <div>
            ...
            <ProButton
              color="primary"
              funcType="raised"
              onClick={() => linkToLDAP(syncSetting)}
            >
              转至LDAP设置
            </ProButton>
          </div>
    ...
    function renderLocked({ value }) {
      return value
        ? formatProjectUser({ id: "lock" })
        : formatProjectUser({ id: "unlock" });
    }
    function rednerEnabled({ value }) {
      return (
        <StatusTag
          name={
            value
              ? formatProjectUser({ id: "action.enable" })
              : formatProjectUser({ id: "action.disable" })
          }
    ...
    function renderUserName({ value, record }) {
      ...
      const service = idEqual
        ? ["choerodon.code.organization.manager.user.ps.update"]
        : [];
      return (
        <span
          style={{
    ...
    function getQueryFields() {
      return {
        ...
        roleName: (
          <Select dropdownMenuStyle={{ width: 260 }} labelLayout="float" />
        ),
    ...
    function renderAction({ record }) {
      let actionDatas = record.get("ldap")
        ...
        : [
            {
              service: [
                "choerodon.code.organization.manager.user.ps.reset.password",
              ],
    ...
    function renderSource({ value }) {
      return formatMessage({
        id: `${intlPrefix}.${value ? "ldap" : "notldap"}`,
      });
    }
          ...
          groupBtnItems: [
            {
              ...
              display: true,
              permissions: [
                "choerodon.code.organization.manager.user.ps.create",
              ],
            ...
            },
            {
              ...
              display: true,
              permissions: [
                "choerodon.code.organization.manager.user.ps.import",
              ],
          ...
          groupBtnItems: [
            {
              name: (
                <Tooltip
                  placement="left"
                  title={<FormattedMessage id={`${intlPrefix}.add.user.tip`} />}
                >
                  {formatProjectUser({ id: "button.assign-roles" })}
                </Tooltip>
              ),
              icon: "person_add-o",
              display: true,
              permissions: [
                "choerodon.code.organization.manager.user.ps.add.user",
              ],
            ...
            },
            {
              name: (
                <Tooltip
                  placement="left"
                  title={
                    <FormattedMessage id={`${intlPrefix}.import.user.tip`} />
                  }
                >
                  {formatProjectUser({
                    id: "button.importingOrganizationUsers",
                  })}
                </Tooltip>
              ),
              icon: "archive-o",
              display: true,
              permissions: [
                "choerodon.code.organization.manager.user.ps.import.user",
              ],
    ...
    return (
      <Page service={permissions}>
        <Header title={<FormattedMessage id={`${intlPrefix}.header.title`} />}>
          <HeaderButtons showClassName={false} items={headerButtons()} />
        </Header>
        <Breadcrumb />
